...
class Widget_CurrentTask extends \xepan\base\Widget{
	function init(){
		parent::init();
	...
	function recursiveRender(){	
	    ...
	    $this->grid->template->trySet('task_view_title','Current Tasks');
	    $this->grid->js('reload')->reload();

		if(!$this->grid->isEditing()){
			$this->grid->grid->template->trySet('task_view_title', 'Current Tasks');
