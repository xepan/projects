...
class Model_Formatted_Project extends \xepan\projects\Model_Project{
	function init(){
		...
		$this->addExpression('total_task')->set($this->refSQL('xepan\projects\Model_Task')->count());
		$this->addExpression('pending_task_count')->set($this->refSQL('xepan\projects\Model_Task')->addCondition('status',['Pending'])->count());
		...
		$this->addExpression('self_task')->set($this->refSQL('xepan\projects\Model_Task')->addCondition('employee_id',$this->app->employee->id)->count());

		$this->addExpression('self_pending_task')->set($this->refSQL('xepan\projects\Model_Task')->addCondition('employee_id',$this->app->employee->id)->addCondition('status',['Pending'])->count());
